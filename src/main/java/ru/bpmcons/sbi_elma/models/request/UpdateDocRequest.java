...
@Data
@Valid
@GroupSequenceProvider(UpdateDocRequest.ValidationProvider.class)
@EqualsAndHashCode(callSuper = true)
public class UpdateDocRequest extends AuthenticatedRequestBase implements Identifiable, FileMetadataContainer {
    ...
    @NotNull
    private Creator_editor editor;                   // автор документа
    @JsonDeserialize(using = StringDeserialization.class)
    ...
    public static final class ValidationProvider implements DefaultGroupSequenceProvider<UpdateDocRequest> {

        @Override
        public List<Class<?>> getValidationGroups(UpdateDocRequest object) {
            ArrayList<Class<?>> seq = new ArrayList<>();
            seq.add(UpdateDocRequest.class);
            if (object == null) {
                return seq;
            }
